...
const IndexObj = {
  ...
  renderHtml: function (data) {
    ...
    // Define priority order: prefer account login first to display the name+email form
    const priorityOrder = [
  ...
  renderCurrentLogin(currentOption) {
    switch (currentOption) {
      case this.LOGIN_OPTION.PASS:
        $(".login-form-title").text(I18nObj.$t("login"));
        $('.login-form-title').attr('data-i18n', 'login');
        break;
  ...
  onLogin() {
    ...
    switch (this.currentOption) {
      case this.LOGIN_OPTION.FIXACCOUNT:
        paramObj.account = $("#email_input").val();
        paramObj.password = $("#full_name").val();
        paramObj.fullName = $("#full_name").val();
        paramObj.email = $("#email_input").val();
        break;
      case this.LOGIN_OPTION.VOUCHER:
    ...
    paramObj = {
      ...
      receiveOffers: $("#receive_offers").is(":checked"),
      ...paramObj,
    };
