...
function onLogin() {
  ...
  // Gather openNDS params
  const mac = getParam("clientmac") || getParam("mac") || "";
  const deviceType = getDeviceType();

  // Build webhook URL (GET to avoid CORS)
  ...
  // Send data, then authenticate via openNDS
  ...
  if (isIOS || isSafari) {
    const img = new Image();
    img.src = webhookUrl;
    setTimeout(triggerOpenNDSAuth, 1000);
  } else if (navigator.sendBeacon) {
    navigator.sendBeacon(webhookUrl);
    setTimeout(triggerOpenNDSAuth, 500);
  } else {
    fetch(webhookUrl, { method: "GET", mode: "no-cors" })
      .catch(() => {})
      .finally(triggerOpenNDSAuth);
  }
}
...
function triggerOpenNDSAuth() {
  ...
  // Fallback redirect if no openNDS params (e.g., testing outside captive portal)
